...
import { LayoutDashboard, FolderOpen, Settings, Lightbulb, X, PanelLeft, PanelLeftClose, Search } from 'lucide-react'
        ...
        <>
            <aside className={`bg-bg-secondary border-r border-glass-border flex flex-col shrink-0 transition-all duration-300 ${isCollapsed ? 'w-20 p-4' : 'w-72 p-6'}`}>
                ...
                <button
                    ...
                    title={isCollapsed ? t('app.search') : undefined}
                    className={`flex items-center rounded-lg text-sm font-semibold transition-colors mb-4 ${isCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-3'} text-text-secondary hover:bg-glass-hover hover:text-white border border-glass-border`}
                >
                    <Search size={20} className="shrink-0" />
                    {!isCollapsed && (
                        <>
                            <span className="flex-1 text-left">{t('app.search')}</span>
                            <span className="text-xs text-text-muted bg-glass px-1.5 py-0.5 rounded">
            ...
            {/* Spotlight Modal (rendered outside sidebar) */}
            <SearchBar />
        </>
    )
}
